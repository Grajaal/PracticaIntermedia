...
public class FichasIguales {
    ...
    public void jugar(){ 
        for(int i = 1; i <= this.numJuegos; i++){
            ...
            for (int j = 1; j <= this.movimientosSolucion.size(); j++) {
                ...
                if(this.movimientosSolucion.get(j - 1).getNumFichasEliminadas() == 1) fichasFormat = "ficha"; 
                else fichasFormat = "fichas";   

                if(this.movimientosSolucion.get(j - 1).getScore() == 1) puntosFormat = "punto"; 
                else puntosFormat = "puntos";   
                System.out.println("Movimiento " + j + " en (" +
                    ...
                    "): eliminó " + this.movimientosSolucion.get(j - 1).getNumFichasEliminadas() + " " + fichasFormat +
                    " de color " + this.movimientosSolucion.get(j - 1).getColor() +
                    " y obtuvo " + this.movimientosSolucion.get(j - 1).getScore() + " " + puntosFormat + "."); 
            }
            String fichasFormat; 
            if(this.tableroSolucion.getNumFichas() == 1) fichasFormat = "ficha"; 
            else fichasFormat = "fichas";
            System.out.println("Puntuación final: " + calcularScore(this.tableroSolucion, this.movimientosSolucion) + ", quedando " + this.tableroSolucion.getNumFichas() + " " + fichasFormat + ".");
            if(i != this.numJuegos) System.out.println();
        }
    }
    ...
    public int calcularScore(Tablero tablero, ArrayList<Movimiento> movimientos){
        ...
        if(tablero.getNumFichas() == 0) score += 1000;  
        return score; 
    }
